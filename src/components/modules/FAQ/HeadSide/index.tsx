...
const HeadSide = async () => {
  const { data: metaData } = await fetchData().getOne<MetadataI>('global_metadata', 'table_name_key', 'faq');

  if (!metaData) return null;

  return (
    <div>
      <h2 className='text-3xl font-semibold md:text-5xl 2xl:text-6xl'>{metaData.title || FAQ}</h2>

...
import { AccordionItemComponentProps } from '@/lib/types/components/modules/FAQ/DataSide/AccordionItemComponent';
...
const AccordionItemComponent = ({ item }: AccordionItemComponentProps) => {
  return (
    <AccordionItem
      title={item.question}
    ...
    >
      ...
      {item.linkText && (
        ...
        >
          ...
          {item.link && <RightArrowUnit link={item.link} />}
        </div>
      )}
    </AccordionItem>
  );
